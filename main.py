...
_VISION_SYSTEM_PROMPT = (
    ...
    'Animals (lost pets) ARE allowed — treat them like any other found item.\n\n'
    'If the image shows a legitimate item or animal suitable for a lost and found service, '
...
def _get_school_public(slug: str) -> dict:
    """Look up school by slug. Returns id, slug, url_token, name, pickup_info, admin_notify_email."""
...
def _school_notify_subscribers_new_item(school: dict, item: dict, extracted: dict) -> None:
    ...
    html = _school_email_html(school["name"], body, slug)
